...
class InformeCampo extends Model
{
    ...
    protected $table = 'informexcampo_new';
    protected $primaryKey = 'infxcam_codigo';
    /**
    ...
    public static function formulario($id)
    {
        ...
        $informeCampos = InformeCampo::where('estado_camp', true)->whereInfCodigo($id)->orderBy('cam_descripcion', 'ASC')->get();
        ...
        foreach ($informeCampos as $infoCampo) {
            $check = View::make('plantillas.checkbox', ['data' => $infoCampo]);
            array_push($resultado['checks'], $check->render());
            if ($infoCampo->cam_filtro) {
    ...
    public static function arrayObjectToCollection($array, $infoCampo)
    {
        ...
        if (is_array($array)) {
            ...
            foreach ($array as $value) {
                $collect->push([
                    'key'=>$value[$infoCampo->opt_value], 'value'=>$value[$infoCampo->opt_contenido]
                ]);
            }
        }
        return $collect;
    ...
    {
        if (($pos = strpos($infoCampo->cam_select_sql, ' AS ')) == 0) {
            $nombreDB = $infoCampo->cam_select_sql;
        } else {
            $nombreDB = substr($infoCampo->cam_select_sql, $pos + 4);
        }
        return $nombreDB;
    }
    ...
    public static function select($infoCampo, $resultado, $nombreDB)
    {
        ...
        $input = View::make('plantillas.select', ['data' => $infoCampo,'name'=>$nombreDB, 'opciones'=>$opciones]);
    ...
    public static function selectMultiple($infoCampo, $resultado, $nombreDB)
    {
        ...
        $input = View::make('plantillas.select_multiple', ['data' => $infoCampo,'name'=>$nombreDB, 'opciones'=>$opciones]);
        array_push($resultado['inputs'], $input->render());
        return $resultado;
    }
    ...
    {
        $input = View::make('plantillas.rango_fechas', ['data' => $infoCampo, 'name'=>$nombreDB ]);
        array_push($resultado['inputs'], $input->render());
        return $resultado;
    }
    ...
    {
        $input = View::make('plantillas.rango_numeros', ['data' => $infoCampo, 'name'=>$nombreDB ]);
        array_push($resultado['inputs'], $input->render());
        return $resultado;
    }
    ...
    {
        $input = View::make('plantillas.text', ['data' => $infoCampo, 'name'=>$nombreDB ]);
        array_push($resultado['inputs'], $input->render());
        return $resultado;
    }
}
